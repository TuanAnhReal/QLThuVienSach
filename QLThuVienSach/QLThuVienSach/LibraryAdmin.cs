...
namespace QLThuVienSach
{
    public class LibraryAdmin
    {
        ...
        public void AddUser(Users user)
        {
            ...
            if (userList.Any(u => u.SDT == user.SDT))
            {
        ...
        public void EditUser(string maKH)
        {
            ...
            if (user != null)
            {
                ...
                user.SDT = Console.ReadLine().Trim();
                Console.Write("Nhập địa chỉ mới: ");
                user.DiaChi = Console.ReadLine();
        ...
        public void SearchUser()//Tìm kiếm thông tin khách hàng 
        {
            ...
            {
                Console.WriteLine("\n════════════════════════════════════════════════════════");
                Console.WriteLine($"|                   THÔNG TIN KHÁCH HÀNG {"",-14}|");
                Console.WriteLine("════════════════════════════════════════════════════════");

                foreach (var user in ketQua)
                {
                    Console.WriteLine($"|Mã KH              :       {user.MaKH,-27}|");
                    Console.WriteLine(new string('-', 56));
                    Console.WriteLine($"|Tên KH             :       {user.TenKH,-26} |");
                    Console.WriteLine(new string('-', 56));
                    Console.WriteLine($"|SDT KH             :       {user.SDT,-26} |");
                    Console.WriteLine(new string('-', 56));
                    Console.WriteLine($"|Địa chỉ KH         :       {user.DiaChi,-26} |");
                    Console.WriteLine(new string('-', 56));
                    Console.WriteLine($"|Trả sách trễ       :       {(user.TrangThaiViPham ? "Có" : "Không"),-26} |");
                    Console.WriteLine(new string('-', 56));
                    ...
                    if (user.DanhSachDaMuon.Count > 0)
                    {
                        foreach (var maSach in user.DanhSachDaMuon)
                        {
                            ...
                            {
                                Console.WriteLine($"{"",-21}{maSach}: {user.ThoiGianMuon[maSach]:dd/MM/yyyy} - {user.ThoiGianTra[maSach]:dd/MM/yyyy}       |");
                            }
                        }
                    }
                    else
                    {
                        Console.WriteLine($"{"",-21} {"Không có!",-33}|");
                    }
                    Console.WriteLine(new string('-', 56));
                    ...
                    if (user.DanhSachDangMuon.Count > 0)
                    {
                        ...
                        foreach (var maSach in user.DanhSachDangMuon)//kết nối thời gian mượn
                        {
                            ...
                            {
                                Console.WriteLine($"{"",-21}{maSach}: {user.ThoiGianMuon[maSach]:dd/MM/yyyy} - {"Chưa trả",-17}|");
                            }
                        }
                    }
                    else
        ...
        public void DisPlayUsers()
        {
            ...
            Console.WriteLine("| Ma KH  | Ten KH          | SDT       | Địa chỉ           | Đã Mượn | Đang Mượn | Vi Phạm  |");
            Console.WriteLine(new string('-', 60));
            foreach (var user in userList)
            {
                ...
                Console.WriteLine($"| {user.MaKH,-6} | {user.TenKH,-15} | {user.SDT,-15} | {user.DiaChi,-15} | {danhSachDaMuon,-7} | {danhSachDangMuon,-9} | {trangThaiViPham,-8} |");
                Console.WriteLine(new string('-', 60));
            }
        }
